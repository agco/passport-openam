...
function createOauth2Strategy(options) {
    ...
    return new OAuth2Strategy(function verify(token, done) {
        ...
        function ducktypeResponse(res, body) {
            // ducktype the response, we expect these values and use them later
            if (!body || !body.agcoUUID || !body.expires_in) return null
            return body
        }
